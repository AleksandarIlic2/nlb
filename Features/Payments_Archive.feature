Feature: Payments_Archive
  ...
  Scenario Outline: Payments-Payments_Archive-Filter_Payments_By_Date_[WEB]
    ...
    And Assert transaction dates in Past payments are between "01.08.2026" and "20.08.2026"
...
##    And Assert list of amounts with currencies of Upcoming payments are displayed correctly
  ...
  Scenario Outline: Payments_Payments_Archive_Payments_Details_[WEB]
    ...
    And Assert field with contains text "Urgent" in payment confirmation matches regex "^(Yes|No)$"
...
#    And Assert label "Name" in payment confirmation contains value from excel "<rowindex>" columnName "account_details_owner"
    ...
    And Assert field "Execution date" in payment confirmation matches regex "^\d{2}\.\d{2}\.\d{4}$"
    And Assert field "Order number" in payment confirmation matches regex "^.{14}$"
#    And Assert label "Name" in payment confirmation contains value from excel "<rowindex>" columnName "auth_current_account_name"
    ...
    And Wait for "2" seconds
    And Click on element by containing text "INTERNAL TRANSFER" and index "2"
    And Wait for element by contains text "Confirmation"
...
#    And Assert label "Name" in payment confirmation contains value from excel "<rowindex>" columnName "account_details_owner"
#    And Assert label "Account number" in payment confirmation contains value from excel "<rowindex>" columnName "current_account_1_iban"
    And Assert field "Address" in payment confirmation matches regex "(?s)^.+$"
    And Assert field "Fee" in payment confirmation matches regex "^\d{1,2},\d{2}\s*RSD$"
    And Assert field "Payment status" in payment confirmation contains text "Executed"
